...
26. **Result CR writes**: After each successful or failed agent invocation (per step), the controller MUST create/update an `AnalysisResult`, `ExecutionResult`, `VerificationResult`, or `EscalationResult` with immutable spec, owner reference to the `AgenticRun`, started/completed conditions, embedded outcome payload, sandbox reference, and optional `failureReason` for system errors.
...
32. **Bare pod lifecycle**: In `bare-pod` mode, for each step invocation the controller MUST create a namespaced `Pod` (core/v1) in the operator namespace with a pod spec assembled by `PodSpecBuilder`. Pod name MUST follow the same `ls-{step}-{agenticRunName}` truncation convention as SandboxClaim naming. Labels MUST include run name and step for identification. After creation, the Pod name MUST be written to `status.steps.<step>.sandbox.claimName` (same field used for SandboxClaim names in `sandbox-claim` mode) so release and log-lookup logic can use a single field regardless of mode.
...
35. **Bare pod release**: In `bare-pod` mode, on run deletion and terminal phases, the controller MUST delete bare Pods by name. Idempotent via NotFound handling.
...
- [PLANNED: OLS-3295] Rename per-run ServiceAccount from `ls-exec-{proposal-namespace}-{proposal-name}` to `ls-exec-{run-namespace}-{run-name}` and update all associated RBAC resource naming.
