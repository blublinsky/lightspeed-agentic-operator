...
4a. **Failed-sandbox debugging**: When a failed `AgenticRun` has the annotation `agentic.openshift.io/preserve-sandbox: "true"`, the controller MUST preserve its sandbox and associated diagnostic resources instead of performing terminal failure cleanup. Terminal TTL processing is disabled for that failed run. Explicit deletion of the `AgenticRun` MUST still perform normal sandbox cleanup.
