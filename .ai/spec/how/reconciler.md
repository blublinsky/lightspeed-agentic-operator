...
- **Tool-result inspection failure [PLANNED: OLS-3928]:** The sandbox exits with a nonzero status, publishes no Result CR, and writes `ToolResultSafetyInspectionFailed` as the termination message. `pod_handler.go` matches this message before generic `SandboxFailed` handling. It uses the same value as the step condition reason. The condition message uses the fixed safe text and excludes tool content.
