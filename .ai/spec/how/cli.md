...
- **`logs`:** Loads the run via the controller-runtime client. Without `--stored`, `resolveSandbox` picks explicit `--step` (normalized via `NormalizeStep`) or prefers verification, then execution, then analysis sandbox info. Uses **`SandboxInfo.ClaimName` as pod name** and `SandboxInfo.Namespace` (fallback run namespace), then streams with optional `-f`. With `--stored`, the command fetches all phases unless an explicit `--step` filter is supplied, then fetches paginated JSON from `/api/v1/logs?agentic_run_id=<uid>&phase=<step>&limit=1000&after=<cursor>` (omitting `phase` when all phases are requested) through the Kubernetes API server's HTTPS Service proxy for `lightspeed-otel-collector` in the AgenticRun/operator namespace, follows all cursors, and renders the Collector-compatible text output with phase headers separating analysis, execution, and verification records. `--admin-endpoint` can override the Service proxy for an externally reachable endpoint. The inherited `--insecure-skip-tls-verify` flag applies to explicit endpoint overrides. Stored logs cannot be followed.
...
- **`watch` terminal set:** `IsTerminalPhase` matches five phases (Completed, Failed, Escalated, Denied, EmergencyStopped); matches all terminal phases defined in `run-lifecycle.md` rule 4.
