  ...
  "properties": {
    ...
    "options": {
      ...
      "items": {
        ...
        "required": ["title", "diagnosis", "remediationPlan", "verification"]
      }
    }
  },
...
func outputSchemaForStep(stepName string, run *agenticv1alpha1.AgenticRun) json.RawMessage {
	if stepName != "analysis" {
		return defaultOutputSchemas[stepName]
	}

	mode := run.Spec.AnalysisOutput.Mode
	if mode == "" {
		mode = agenticv1alpha1.AnalysisOutputModeDefault
	}
	customSchema := run.Spec.AnalysisOutput.Schema
	...
	if mode == agenticv1alpha1.AnalysisOutputModeDefault {
		required = append(required, "diagnosis", "remediationPlan")
	}

	if !run.Spec.Execution.IsZero() {
		if mode == agenticv1alpha1.AnalysisOutputModeMinimal {
			injectBuiltInProperty(props, "remediationPlan")
			injectBuiltInProperty(props, "rbac")
			required = append(required, "remediationPlan")
		}
