...
func NewDenyCmd(streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "deny NAME",
		Short: "Deny a run step",
		Example: `  # Deny the execution step
  oc agentic run deny fix-crash --stage=execution

  # Deny the next pending step
  oc agentic run deny fix-crash`,
...
func (o *DenyOptions) Run(ctx context.Context) error {
	...
	if err := o.client.Get(ctx, types.NamespacedName{Name: o.name, Namespace: o.namespace}, approval); err != nil {
		return fmt.Errorf("failed to get AgenticRunApproval %q: %w", o.name, err)
	}
	...
	fmt.Fprintf(o.Out, "run/%s stage %s denied\n", o.name, stageName)
	return nil
}

func (o *DenyOptions) nextPendingStage(p *agenticv1alpha1.AgenticRun, approval *agenticv1alpha1.AgenticRunApproval) string {
